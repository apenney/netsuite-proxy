...
class NetSuiteConfig(BaseSettings):
    ...
    def validate_oauth_completeness(self) -> "NetSuiteConfig":
    ...
    @model_validator(mode="after")
    def validate_restlet_config(self) -> "NetSuiteConfig":
        """Ensure RESTlet configuration is complete if provided."""
        if (self.script_id is None) != (self.deploy_id is None):
            raise ValueError("Both script_id and deploy_id must be provided for RESTlet configuration")
        return self
